...
function dphimg(source_images_dir, resized_images_cache_dir, imagelist) {
	...
	function __readFile(err, imagedata, width, height, emitter) {
        ...
        var destination = resized_images_cache_dir + '/' + filename;
        console.log("generating image: ", destination)
				generateResizedImage(destination, imagedata, width, height, emitter);
			}
		});
